...
onMounted(() => {
  for (
    let idx = 0;
    attrs.modelValue !== undefined && idx < attrs.modelValue.length;
    idx++
  ) {
    userCheckedbox.value[idx] = attrs.modelValue[idx];
  }
  ...
  let storageItem = $q.localStorage.getItem(searchNameStorage);

  // alert("storageItem: " + storageItem);
  // 스토리지에 저장된 값이 없으면 빈 배열로 세팅(오류 방지)
  if (storageItem === null || storageItem === "") {
    $q.localStorage.set(searchNameStorage, []);
  }

  // 세팅된 검색값을 부모 컴포넌트에  emit
  emit(
    "update:model-value",
    $q.localStorage.getItem(searchNamePrefix + props.searchName)
  );
